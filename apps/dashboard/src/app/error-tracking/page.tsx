		...
		<main id="main-content" className="mx-auto flex max-w-5xl flex-col gap-6 p-4 sm:p-8">
			<header className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
				<PageTitle
					...
					subtitle={<p className="text-muted-foreground text-sm">Últimas {minutesAgo} minuto(s), ordenado por volume</p>}
				/>
			</header>
			...
			<FilterForm
				...
				values={{ query, since }}
				fields={[
					{ name: 'query', label: 'Busca (formato Datadog)', type: 'text', placeholder: 'service:lello-web env:production' },
			...
			{issues.length === 0 ? (
				...
				<>
					...
					<Pagination page={page} totalPages={totalPages} basePath="/error-tracking" params={{ query, since }} />
				</>
			)}
		</main>
	);
