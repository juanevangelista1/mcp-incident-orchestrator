...
import { summarizeDatadogLogsInputSchema } from '../datadog.schema.js';
...
export function registerSummarizeDatadogLogsTool(server: McpServer, datadogService: DatadogService): void {
	server.tool(
		...
		'Gera um resumo dos logs recentes: total, quebra por status (error/warn/info) e por serviço.',
		summarizeDatadogLogsInputSchema.shape,
		async (args) => {
			try {
